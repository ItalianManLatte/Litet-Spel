...
bool CreateDepthStencil(ID3D11Device* device, UINT width, UINT height, ID3D11Texture2D*& dsTexture, ID3D11DepthStencilView*& dsView)
{
	...
	{
		std::cerr << "Falied to create depth stencil texture" << std::endl;
		return false;
	}
...
bool SetupD3D11(UINT width,	UINT height, HWND window, ID3D11Device*& device, ID3D11DeviceContext*& immediateContext,
	IDXGISwapChain*& swapChain,	ID3D11RenderTargetView*& rtv, ID3D11Texture2D*& dsTexture, ID3D11DepthStencilView*& dsView,
	D3D11_VIEWPORT& viewport)
{
	if (!CreateInterface(width, height, window, device, immediateContext, swapChain))
	{
		std::cerr << "Error: failed to create the interface" << std::endl;
		return false;
	}

	if (!CreateRenderTargetView(device, swapChain, rtv))
	{
		std::cerr << "Error: failed to create the rtv" << std::endl;
		return false;
	}

	if (!CreateDepthStencil(device, width, height, dsTexture, dsView))
	{
		std::cerr << "Error: failed to create the depth stencil view" << std::endl;
		return false;
	}
